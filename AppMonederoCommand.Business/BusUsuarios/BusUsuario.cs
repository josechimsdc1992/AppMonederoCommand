...
namespace AppMonederoCommand.Business.BusUsuarios
{
    public class BusUsuario : IBusUsuario
    {
        ...
        public async Task<IMDResponse<dynamic>> BEnviaCorreoRecuperacion(EntRecuperaCuenta rescuperaCuenta)
        {
            IMDResponse<dynamic> response = new IMDResponse<dynamic>();

            string metodo = nameof(this.BEnviaCorreoRecuperacion);
            _logger.LogInformation(IMDSerializer.Serialize(67823462667583, $"Inicia {metodo}(EntRecuperaCuenta rescuperaCuenta)", rescuperaCuenta));

            try
            {
                if (string.IsNullOrEmpty(rescuperaCuenta.sCorreo))
                {
                    response.SetError(Menssages.BusRequiredEmailRestore);
                }
                else
                {

                    string sCorreo = rescuperaCuenta.sCorreo;

                    var existeCuenta = await _datUsuario.DGetExisteCuenta(sCorreo);

                    if (existeCuenta.HasError != true)
                    {
                        var tokenRecuperacion = await BGeneraTokenRecuperacion();
                        if (tokenRecuperacion.HasError != true)
                        {

                            //Envio de correo 
                            string nombreUsuario = existeCuenta.Result.sNombre + " " + existeCuenta.Result.sApellidoPaterno + " " + existeCuenta.Result.sApellidoMaterno;
                            string plantillaRecuperaPass = string.Empty;

                            try
                            {
                                plantillaRecuperaPass = _busLenguaje.BusSetLanguajeRecuperarPass();
                            }
                            catch (Exception ex)
                            {
                                response.ErrorCode = 67823462668360;
                                response.SetError($"{Menssages.BusPlantillaError}: {ex}");
                                _logger.LogError(IMDSerializer.Serialize(67823462668360, $"Error en {metodo}(EntRecuperaCuenta rescuperaCuenta): {ex.Message}", rescuperaCuenta, ex, response));
                                return response;
                            }

                            var correoRecuperaCuenta = plantillaRecuperaPass.Replace("{Nombre}", nombreUsuario.Trim());
                            correoRecuperaCuenta = correoRecuperaCuenta.Replace("{correo}", existeCuenta.Result.sCorreo);
                            correoRecuperaCuenta = correoRecuperaCuenta.Replace("{token}", tokenRecuperacion.Result);

                            Guid uIdUsuario = existeCuenta.Result.uIdUsuario;
                            sCorreo = existeCuenta.Result.sCorreo;
                            string sTokenRecuperacion = tokenRecuperacion.Result;
                            sTokenRecuperacion = sTokenRecuperacion.Replace("/", "+");

                            EntHistorialRecuperacionToken historialToken = new EntHistorialRecuperacionToken
                            {
                                uIdUsuario = uIdUsuario,
                                sCorreo = sCorreo,
                                sToken = sTokenRecuperacion,
                                sNombre = nombreUsuario,
                                dtFechaCreacion = DateTime.UtcNow,
                                dtFechaVencimiento = DateTime.UtcNow.AddMinutes(30),
                                bActivo = true
                            };

                            var tokenGuardado = await _datUsuario.DSaveTokenRecuperacion(historialToken);

                            if (tokenGuardado.HasError != true)
                            {
                                EntBusMessCorreo busMessage = new EntBusMessCorreo
                                {
                                    uIdUsuario = uIdUsuario,
                                ...
                                response.SetSuccess(Menssages.BusComplete, Menssages.BusCorrecto);
                            }
                            else
                            {
                                response.ErrorCode = tokenGuardado.ErrorCode;
                                response.SetError(tokenGuardado.Message);
                            }
                        }
                        else
                        {
                    ...
                    {
                        if (existeCuenta.HttpCode == HttpStatusCode.NotFound)
                        {
                            response.SetNotFound(existeCuenta.Result, existeCuenta.Message);
                        }
                        else
                        {
                            response.ErrorCode = existeCuenta.ErrorCode;
        ...
        //Cambia contraseña desde link de recuperación
        public async Task<IMDResponse<dynamic>> BRecuperaCambiaContrasena(string sCorreo, string sToken, EntNuevaContrasena contrasena)
        {
            IMDResponse<dynamic> response = new IMDResponse<dynamic>();

            string metodo = nameof(this.BRecuperaCambiaContrasena);
            _logger.LogInformation(IMDSerializer.Serialize(67823462703325, $"Inicia {metodo}(string sCorreo, string sToken, EntNuevaContrasena contrasena)", sCorreo, sToken, contrasena));
            ...
            try
            {
                ...
                //Valida si existe cuenta
                if (existeCuenta.HasError != true)
                {
                    //Valida si existe/valido el token
                    var tokenValido = await _datUsuario.DGetTokenValido(existeCuenta.Result.uIdUsuario, sCorreo, sToken);

                    if (tokenValido.HasError != true && tokenValido.Result == true)
                    {
                        if (contrasena.sContrasenia.Equals(contrasena.sConfirmaContrasenia) == true)
                        {
                            ...
                            var actualizaHistorialRecuperacionToken = await _datUsuario.DUpdateHistorialRecuperacionToken(existeCuenta.Result.uIdUsuario, sToken);

                            if (actualizaHistorialRecuperacionToken.HasError != true)
                            {
                                var actualizaContrasena = await _datUsuario.DUpdateContrasena(existeCuenta.Result.uIdUsuario, contrasena, sToken);
                            ...
                            {
                                response.ErrorCode = actualizaHistorialRecuperacionToken.ErrorCode;
                                response.SetError(actualizaHistorialRecuperacionToken.Message);
                            }
                    ...
                    {
                        response.ErrorCode = tokenValido.ErrorCode;
                        response.SetError(tokenValido.Message);
                    }
                }
                else
                {
            ...
            {
                response.ErrorCode = 67823462704102;
                response.SetError(ex);

                _logger.LogError(IMDSerializer.Serialize(67823462704102, $"Error en {metodo}(string sCorreo, string sToken, EntNuevaContrasena contrasena): {ex.Message}", sCorreo, sToken, contrasena, ex, response));
            }
            return response;
        }

        //Valida que el token de recuperar contraseña sea válido
        public async Task<IMDResponse<EntHistorialRecuperacionToken>> BValidarTokenRecuperacion(string correo, string token)
        {
            IMDResponse<EntHistorialRecuperacionToken> response = new IMDResponse<EntHistorialRecuperacionToken>();

            string metodo = nameof(this.BValidarTokenRecuperacion);
            _logger.LogInformation(IMDSerializer.Serialize(67823462745283, $"Inicia {metodo}(string correo, string token)", correo, token));

            try
            {
                var existeCuenta = await _datUsuario.DGetExisteCuenta(correo);

                if (!existeCuenta.HasError)
                {
                    //Valida si existe/valido el token
                    var tokenValido = await _datUsuario.DGetTokenValido(existeCuenta.Result.uIdUsuario, correo, token);
                    if (tokenValido.Result)
                    {
                        var datosHistorial = await _datUsuario.DGetHistorialTokenRecuperarCuenta(correo, token);
                        if (!datosHistorial.HasError)
                        {
                            response.SetSuccess(datosHistorial.Result);
                        }
                        else
                        {
                            response.SetNotFound(new EntHistorialRecuperacionToken(), Menssages.BusNotfoundHistory);
                        }
                    }
                    else
                    {
                        response.SetNotFound(new EntHistorialRecuperacionToken(), Menssages.BusTokenNoValid);
                        _logger.LogError(IMDSerializer.Serialize(67823462746060, $"Error en {metodo}(string sCorreo, string sToken)", correo, token, response));
                    }
                }
                else
                {
                    response.SetNotFound(new EntHistorialRecuperacionToken(), Menssages.DatRegisterNoExist);
                    _logger.LogError(IMDSerializer.Serialize(67823462746060, $"Error en {metodo}(string sCorreo, string sToken)", correo, token, response));
                }
            }
            catch (Exception ex)
            {
                response.ErrorCode = 67823462746060;
                response.SetError(ex.Message);

                _logger.LogError(IMDSerializer.Serialize(67823462746060, $"Error en {metodo}(string correo, string token): {ex.Message}", correo, token, ex, response));
            }
            return response;
        }
        ...
        [IMDMetodo(67823463464008, 67823463463231)]
        public async Task<IMDResponse<EntUsuarioResponse>> BUpdateUsuarioAdicional(EntUsuarioActualizaTelefonoRequest entUsuario, Guid uIdUsuario)
        {
            IMDResponse<EntUsuarioResponse> response = new IMDResponse<EntUsuarioResponse>();

            IMDMetodo metodo = MethodBase.GetCurrentMethod()!.GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(67823462597653, $"Inicia {metodo}(EntUsuarioActualizaTelefonoRequest entUsuario, Guid uIdUsuario)", entUsuario, uIdUsuario));

            try
            {

                var datosCompletos = await BValidaDatosUsuarioActualizaTelefono(entUsuario);
                if (datosCompletos.HasError)
                {
                    response.ErrorCode = datosCompletos.ErrorCode;
                    response.SetError(datosCompletos.Message);
                ...
                //valida estructura de correo
                IMDResponse<bool> correoValido = await BValidaEmail(entUsuario.sCorreo!);
                if (datosCompletos.HasError)
                {
                    response.ErrorCode = correoValido.ErrorCode;
                    response.SetError(correoValido.Message);
                ...
                IMDResponse<bool> existeTelefono = await BExisteTelefono(entUsuario.sTelefono!);
                if (existeTelefono.Result)
                {
                    response.SetError(Menssages.BusPhoneNoValid);
                    return response;
                ...
                IMDResponse<bool> existeCURP = await BExisteCURP(entUsuario.sCURP!);
                if (existeCURP.Result)
                ...
                var getByIdUsuarioResponse = await _datUsuarioActualizaTelefono.DGetByIdUsuario(uIdUsuario);

                if (!getByIdUsuarioResponse.HasError)
                {
                    ...
                    var saveResponse = new IMDResponse<bool>();
                    if (getByIdUsuarioResponse.Result != null)
                    {
                        saveResponse = await _datUsuarioActualizaTelefono.DUpdate(entUsuario, uIdUsuario);
                    }
                    else
                    {
                        saveResponse = await _datUsuarioActualizaTelefono.DSave(entUsuario, uIdUsuario);
                    }
                    ...
                    if (saveResponse.Result)
                    {
                        ...
                        try
                        {
                            ...
                            if (sendCodeByEmail)
                            {
                                var user = await BGet(uIdUsuario);
                                string nombre = $"{user.Result.sNombre} {user.Result.sApellidoPaterno} {user.Result.sApellidoMaterno}";
                                await EnviarCodigoVerificacionEmail(uIdUsuario, entUsuario.sCorreo!, codigoVerificacion.Result, nombre.Trim());
                            }
                        }
                        catch (Exception ex)
                        {
                            _logger.LogError(IMDSerializer.Serialize(metodo.iCodigoError, $"Error en {metodo}(EntUsuarioActualizaTelefonoRequest entUsuario, Guid uIdUsuario): {ex.Message}", entUsuario, uIdUsuario, ex, response));
                        }

                        EntBusMessUsuarioCodigoVerificacion busMessage = new EntBusMessUsuarioCodigoVerificacion
                        {
                            uIdUsuario = uIdUsuario,
                            sNumeroTelefono = $"+52{entUsuario.sTelefono}",
                            sMensaje = $"{Menssages.BusCodeVerification}: {codigoVerificacion.Result}"
                        };
                        ///Envío a bus evento Nueva Cuenta Código de Verificacion  
                        await _rabbitNotifications.SendAsync(RoutingKeys.NotificacionSms.GetDescription(), _exchangeConfig, new QueueMessage<EntBusMessUsuarioCodigoVerificacion>
                        ...
                        //Se actualiza datos de monedero
                        //await BActualizarDatosMonedero(uIdUsuario);

                        response.SetSuccess(entUsuarioResponse, Menssages.BusCompleteCorrect);
                    }
                    else
                    {
                        response.SetError(saveResponse.Message);
                    }
                }
            }
            catch (Exception ex)
        ...
        [IMDMetodo(67823463465562, 67823463464785)]
        public async Task<IMDResponse<EntUsuarioResponse>> BReenviarCodigoActualizarTelefono(Guid uIdUsuario)
        {
            IMDResponse<EntUsuarioResponse> response = new IMDResponse<EntUsuarioResponse>();
            ...
            try
            {
                ...
                if (!getByIdUsuarioResponse.HasError)
                {
                    if (getByIdUsuarioResponse.Result != null)
                    {
                        var usuario = getByIdUsuarioResponse.Result;

                        var entUsuario = new EntUsuarioActualizaTelefonoRequest();
                        entUsuario.sCURP = usuario.sCURP;
                        entUsuario.sTelefono = usuario.sTelefono;
                        entUsuario.sCorreo = usuario.sCorreo;
                        entUsuario.sCodigoVerificacion = codgioEncrypt;

                        var updateResponse = await _datUsuarioActualizaTelefono.DUpdate(entUsuario, uIdUsuario);
                        if (updateResponse.Result)
                        {
                            //Se actualiza datos de monedero
                            var user = await _datUsuario.DGetUsuario(uIdUsuario);
                            ...
                            else
                            {
                                ...
                                response.SetError(user.Message);
                                return response;
                            }
                            //Termina actualización de monedero
                            EntUsuarioResponse entUsuarioResponse = new EntUsuarioResponse();
                            ...
                            try
                            {
                                ...
                                {
                                    string nombre = $"{user.Result.sNombre} {user.Result.sApellidoPaterno} {user.Result.sApellidoMaterno}";
                                    await EnviarCodigoVerificacionEmail(uIdUsuario, entUsuario.sCorreo!, generaCodigo.Result, nombre);
                                }
                            }
                            catch (Exception ex)
                            {
                            ...
                            {
                                uIdUsuario = uIdUsuario,
                                sNumeroTelefono = $"+52{getByIdUsuarioResponse.Result.sTelefono}",
                                sMensaje = $"{Menssages.BusCodeVerification}: {generaCodigo.Result}"
                            };
                            ///Envío a bus evento Nueva Cuenta Código de Verificacion  
                            await _rabbitNotifications.SendAsync(RoutingKeys.NotificacionSms.GetDescription(), _exchangeConfig, new QueueMessage<EntBusMessUsuarioCodigoVerificacion>
                            ...
                            response.SetSuccess(entUsuarioResponse, Menssages.BusSMSSend);
                        }
                        else
                        {
                            response.SetError(updateResponse.Message);
                        }
                    }
                }
                else
                {
                    response.SetError(getByIdUsuarioResponse.Message);
                }
            }
            catch (Exception ex)
            {
        ...
        [IMDMetodo(67823463462454, 67823463461677)]
        public async Task<IMDResponse<dynamic>> BVerificaCodigoActualizarTelefono(EntCodigoVerificacion codigo, Guid uIdUsuario)
        {
            IMDResponse<dynamic> response = new IMDResponse<dynamic>();

            IMDMetodo metodo = MethodBase.GetCurrentMethod()!.GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(metodo.iCodigoInformacion, $"Inicia {metodo}(EntCodigoVerificacion codigo, Guid uIdUsuario)", codigo, uIdUsuario));
            ...
            try
            {
                ...
                if (!getByIdUsuarioResponse.HasError)
                {
                    ...
                    if (usuario != null)
                    {
                        string PCKEY = Environment.GetEnvironmentVariable("PCKEY") ?? "";
                        string PCIV = Environment.GetEnvironmentVariable("PCIV") ?? "";
                        var codigoValido = IMDSecurity.BEncrypt(codigo.sClaveVerificacion!, PCKEY, PCIV);

                        if (usuario.sCodigoVerificacion != codigoValido)
                        {
                            response.SetNotFound(codigo);
                            return response;
                        }

                        var updateVerificadoResponse = await _datUsuarioActualizaTelefono.DVerificado(uIdUsuario);
                        if (updateVerificadoResponse.HasError)
                        {
                            response.ErrorCode = updateVerificadoResponse.ErrorCode;
                            response.SetError(updateVerificadoResponse.Message);
                            return response;
                        }

                        var updateUserResponse = await _datUsuario.DUpdateUsuarioDatosAdicionales(uIdUsuario, usuario.sTelefono!, usuario.sCURP!);
                        if (updateUserResponse.HasError)
                        {
                            response.ErrorCode = updateUserResponse.ErrorCode;
                            response.SetError(updateUserResponse.Message);
                            return response;
                        }

                        EntCodigoResponse entCodigoResponse = new EntCodigoResponse();
                        var entUsuario = updateUserResponse.Result;
                        if (entUsuario != null)
                        {
                            //Se actualiza datos de monedero
                            ...
                            if (entUsuario.uIdMonedero != null)
                            {
                                var monederoActualizado = await BActualizaDatosMonedero(entUsuario);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(IMDSerializer.Serialize(metodo.iCodigoError, $"Error en {metodo}(EntCodigoVerificacion codigo, Guid uIdUsuario): {ex.Message}", codigo, uIdUsuario, ex, response));
            }
            return response;
        }
        ...
        [IMDMetodo(67823465602312, 67823465601535)]
        public async Task<IMDResponse<EntPagination<EntUsuarioApp>>> BGetUsuariosApp(EntFiltersUsuario pEntity, string sToken)
        {
            IMDResponse<EntPagination<EntUsuarioApp>> response = new IMDResponse<EntPagination<EntUsuarioApp>>();
            IMDMetodo metodo = MethodBase.GetCurrentMethod()!.GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(metodo.iCodigoInformacion, $"Inicia {metodo}(EntFiltersUsuario pEntity, string sToken)", pEntity, sToken));

            try
            {
                IMDResponse<EntDatosTarjeta> monedero = new IMDResponse<EntDatosTarjeta>();
                if (!string.IsNullOrEmpty(pEntity.sNumMonederoTarjeta))
                    monedero = await _busMonedero.BGetDatosByNumMonedero(pEntity.sNumMonederoTarjeta, sToken);
                else
                    monedero.HasError = true;

                if (!monedero.HasError)
                {
                    pEntity.uIdMonedero = monedero.Result.uIdMonedero;
                }
                else
                {
                    if (!string.IsNullOrEmpty(pEntity.sNumMonederoTarjeta))
                    {
                        var tarjeta = await _busTarjetaUsuario.BGetTarjetaByNumTarjeta(pEntity.sNumMonederoTarjeta);
                        if (!tarjeta.HasError)
                        {
                            pEntity.uIdUsuario = tarjeta.Result.uIdUsuario;
                        }
                        else
                        {
                            var paginator = new EntPagination<EntUsuarioApp>(pEntity.iPage, pEntity.iRegistros, 0);
                            paginator.Datos = new List<EntUsuarioApp>();
                ...
                response = await _datUsuario.DGetUsuariosApp(pEntity);
            }
            catch (Exception ex)
            {
                response.ErrorCode = metodo.iCodigoError;
                ...
                _logger.LogError(IMDSerializer.Serialize(metodo.iCodigoError, $"Error en {metodo}(EntFiltersUsuario pEntity, string sToken): {ex.Message}", pEntity, sToken, ex, response));
            }
            return response;
        }

        [IMDMetodo(67823465606974, 67823465606197)]
        public async Task<IMDResponse<CSVResponse>> BGetUsuariosAppCSV(EntFiltersUsuario pEntity, string sToken)
        {
            IMDResponse<CSVResponse> response = new IMDResponse<CSVResponse>();
            IMDMetodo metodo = MethodBase.GetCurrentMethod()!.GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(metodo.iCodigoInformacion, $"Inicia {metodo}(EntFiltersUsuario pEntity, string sToken)", pEntity, sToken));
            ...
            try
            {
                ...
                var usuariosApp = await _datUsuario.DGetUsuariosApp(pEntity);
                if (usuariosApp.Result.iTotalRegistros != 0)
                {
                    var reporte = usuariosApp.Result.Datos!.Select(item => new EntUsuarioAppCSV
                    {
                        sNombreCompleto = item.sNombreCompleto,
                        sLada = item.sLada,
                        sTelefono = item.sTelefono,
                        sCURP = item.sCURP,
                        sFechaCreacion = item.dtFechaCreacion != null ? "\t" + item.dtFechaCreacion.ToString("dd/MM/yyyy HH:mm:ss") : string.Empty,
                        sCorreo = item.sCorreo,
                        sMigrado = item.bMigrado ? "SI" : "NO",
                        sTieneMonedero = item.bMonedero ? "SI" : "NO"
                    }).ToList();

                    byte[] _bytes = GenerateCSV.GenerarArchivoCSV(reporte);
                    response.SetSuccess(new CSVResponse
                    {
                        bytes = _bytes,
                        sName = "Reporte_Usuarios_App.csv"
                    });
                }
                else
                {
                    response.GetNotFound("No se encontró información");
                }
            }
            catch (Exception ex)
            {
        ...
        [IMDMetodo(67823466286849, 67823466287626)]
        public async Task<IMDResponse<EntCodigoCuentaResponse>> BEnviarCodigoEliminacionCuenta(EntCodigoCuentaRequest entCodigoCuentaRequest)
        {
            IMDResponse<EntCodigoCuentaResponse> response = new IMDResponse<EntCodigoCuentaResponse>();

            IMDMetodo metodo = MethodBase.GetCurrentMethod().GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(metodo.iCodigoInformacion, $"Inicia {metodo.sNombre}{metodo.sParametros}", entCodigoCuentaRequest));
            try
            {
                var existeCuenta = await _datUsuario.DGetExisteCuenta(entCodigoCuentaRequest.sCorreo);

                // Genera Código Alfanumérico
                var codeResponse = await BGeneraCodigoAlfanumerico(4);

                if (!codeResponse.Result.IsNullOrEmpty())
                {
                    if (existeCuenta.HttpCode == HttpStatusCode.OK)
                        await EnviarCodigoEliminacionCuenta(existeCuenta.Result.uIdUsuario, entCodigoCuentaRequest.sCorreo, codeResponse.Result);

                    EntDataToEncrypt entDataToEncrypt = new EntDataToEncrypt()
                    {
                        sCode = codeResponse.Result,
                        dtFechaVigencia = DateTime.Now,
                        uIdUsuario = existeCuenta.Result.uIdUsuario
                    };

                    string jsonDataToEncrypt = JsonConvert.SerializeObject(entDataToEncrypt);

                    var encryptedDataResponse = await BEncryptData(jsonDataToEncrypt, true);
                    string sEncryptedData = encryptedDataResponse.Result;

                    EntCodigoCuentaResponse entCodigoCuentaResponse = new EntCodigoCuentaResponse()
                    {
                        sCorreo = entCodigoCuentaRequest.sCorreo,
                        sEncryptedData = sEncryptedData
                    };

                    response.SetSuccess(entCodigoCuentaResponse);
                }
                else
                {
                    response.ErrorCode = metodo.iCodigoError;
                    response.SetError(codeResponse.Message);
                }
            }
            catch (Exception ex)
            {
                ...
                _logger.LogError(IMDSerializer.Serialize(metodo.iCodigoError, $"Error en {metodo.sNombre}{metodo.sParametros}: {ex.Message}", entCodigoCuentaRequest, ex, response));
            }
            return response;
        }

        [IMDMetodo(67823466280633, 67823466281410)]
        public async Task<IMDResponse<bool>> BConfirmarEliminacionCuenta(EntEliminaCuentaRequest entEliminaCuentaRequest)
        {
            IMDResponse<bool> response = new IMDResponse<bool>();

            IMDMetodo metodo = MethodBase.GetCurrentMethod().GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(metodo.iCodigoInformacion, $"Inicia {metodo.sNombre}{metodo.sParametros}", entEliminaCuentaRequest));
            try
            {
                // Desencriptar cadena
                var decryptDataResponse = await BEncryptData(entEliminaCuentaRequest.sEncryptedData, false);

                // Deserializar datos
                EntDataToEncrypt entDataDecrypt = JsonConvert.DeserializeObject<EntDataToEncrypt>(decryptDataResponse.Result);

                if (entEliminaCuentaRequest.sCode != entDataDecrypt.sCode)
                {
                    response.ErrorCode = metodo.iCodigoError;
                    response.SetError(Menssages.CodeNotMatch);
                    return response;
                }

                var parametroResponse = await _busParametros.BObtener("VIGENCIA_TOKEN_CANCELACION");
                int minutosVigencia = int.Parse(parametroResponse.Result.sValor ?? "30");

                DateTime currentTimeServer = DateTime.Now;
                DateTime dtVigencia = entDataDecrypt.dtFechaVigencia.AddMinutes(minutosVigencia);
                if (currentTimeServer > dtVigencia)
                {
                    response.ErrorCode = metodo.iCodigoError;
                    response.SetError(Menssages.CodeTimeExpired);
                    return response;
                }

                // Se procede a eliminar
                response = await BEliminarUsuario(entDataDecrypt.uIdUsuario);
            }
            catch (Exception ex)
            {
                response.ErrorCode = 500;
                ...
                _logger.LogError(IMDSerializer.Serialize(metodo.iCodigoError, $"Error en {metodo.sNombre}{metodo.sParametros}: {ex.Message}", entEliminaCuentaRequest, ex, response));
            }
            return response;
        }

        [IMDMetodo(67823466289957, 67823466290734)]
        public async Task<IMDResponse<bool>> BSolicitarCambioDispositivo(EntEstatusCuentaRequest entEstatusCuentaRequest)
        {
            IMDResponse<bool> response = new IMDResponse<bool>();

            IMDMetodo metodo = MethodBase.GetCurrentMethod()!.GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(metodo.iCodigoInformacion, $"Inicia {metodo.sNombre}{metodo.sParametros}", entEstatusCuentaRequest));
            try
            {
                var idTarifaGeneral = _busParametros.BObtener("APP_TARIFAGENERAL_GUID").Result.Result.sValor ?? Guid.Empty.ToString();

                if (!idTarifaGeneral.IsNullOrEmpty() && idTarifaGeneral == entEstatusCuentaRequest.uIdTipoTarifa.ToString())
                {
                    response.ErrorCode = metodo.iCodigoError;
                    response.SetError(Menssages.OperationNotAllowed);
                    return response;
                }

                EntEstatusCuentaUpdate entEstatusCuentaUpdate = new EntEstatusCuentaUpdate()
                {
                    uIdUsuario = entEstatusCuentaRequest.uIdUsuario,
                    uIdTipoTarifa = entEstatusCuentaRequest.uIdTipoTarifa,
                    iEstatusCuenta = (int)eEstatusCuenta.ENCAMBIO
                };

                response = await BActualizaEstatusCuenta(entEstatusCuentaUpdate);
            }
            catch (Exception ex)
            {
                response.ErrorCode = 500;
                ...
                _logger.LogError(IMDSerializer.Serialize(metodo.iCodigoError, $"Error en {metodo.sNombre}{metodo.sParametros}: {ex.Message}", entEstatusCuentaRequest, ex, response));
            }
            return response;
        }

        [IMDMetodo(67823466291511, 67823466292288)]
        public async Task<IMDResponse<bool>> BActualizaEstatusCuenta(EntEstatusCuentaUpdate entEstatusCuentaUpdate)
        {
            IMDResponse<bool> response = new IMDResponse<bool>();
            ...
            try
            {
                response = await _datUsuario.DActualizaEstatusCuenta(entEstatusCuentaUpdate);
            }
            catch (Exception ex)
            {
                response.ErrorCode = 500;
        ...
        [IMDMetodo(67823466356779, 67823466357556)]
        public async Task<IMDResponse<dynamic>> BVerificaCodigoDispositivo(EntCodigoVerificacion entCodigoVerificacion)
        {
            IMDResponse<dynamic> response = new IMDResponse<dynamic>();

            IMDMetodo metodo = MethodBase.GetCurrentMethod()!.GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(metodo.iCodigoInformacion, $"Inicia {metodo.sNombre}{metodo.sParametros}", entCodigoVerificacion));
            try
            {
                var parametroResponse = await _busParametros.BObtener("APP_VIGENCIA_CODIGO");
                int minutosVigencia = int.Parse(parametroResponse.Result.sValor ?? "5");
                bool bTestStore = bool.Parse(_busParametros.BObtener("APP_TEST_STORE").Result.Result.sValor ?? "false");
                string sTestStoreCode = _busParametros.BObtener("APP_TEST_STORE_CODE").Result.Result.sValor;
                string sTestStoreEmail = _busParametros.BObtener("APP_TEST_STORE_EMAIL").Result.Result.sValor;

                IMDResponse<EntUsuario> usuarioResponse = await BGetByCorreo(entCodigoVerificacion.sCorreo);


                if (usuarioResponse.HasError)
                {
                    response.ErrorCode = usuarioResponse.ErrorCode;
                    response.SetError(usuarioResponse.Message);
                    return response;
                }

                if (!string.IsNullOrEmpty(entCodigoVerificacion.sCorreo) && entCodigoVerificacion.sCorreo == sTestStoreEmail && bTestStore)
                {
                    if (entCodigoVerificacion.sClaveVerificacion == sTestStoreCode)
                    {
                        EntDispositivoCuentaUpdate entDispositivoCuentaUpdate = new EntDispositivoCuentaUpdate()
                        {
                            uIdUsuario = usuarioResponse.Result.uIdUsuario,
                            sIdAplicacion = entCodigoVerificacion.sIdAplicacion,
                            iEstatusCuenta = (int)eEstatusCuenta.DESBLOQUEADO
                        };
                ...
                if (usuarioResponse.Result != null)
                {
                    DateTime currentTimeServer = DateTime.Now;
                    DateTime dtVigencia = usuarioResponse.Result.dtFechaModificacion!.Value.AddMinutes(minutosVigencia);
                    ...
                    // Desencriptar código
                    var decryptCodeResponse = await BEncryptData(usuarioResponse.Result.sCodigoVerificacion!, false);

                    if (decryptCodeResponse.Result != entCodigoVerificacion.sClaveVerificacion)
                    {
                        response.ErrorCode = metodo.iCodigoError;
                        response.SetError(Menssages.CodeNotMatch);
                        response.Result = false;
                    ...
                    var updateResponse = BActualizaDispositivoCuenta(entDispositivoCuentaUpdate).Result;
                    ...
                    #region Cerrar Sesión Otros Dispositivos
                    BCerrarSesionDispositivos(entDispositivoCuentaUpdate);
                    #endregion
                ...
                {
                    response.SetNotFound(usuarioResponse, Menssages.DatUserNoExist);
                }
            }
            catch (Exception ex)
            {
                ...
                _logger.LogError(IMDSerializer.Serialize(metodo.iCodigoError, $"Error en {metodo.sNombre}{metodo.sParametros}: {ex.Message}", entCodigoVerificacion, ex, response));
            }

            return response;
        }

        [IMDMetodo(67823466381643, 67823466382420)]
        public async Task<IMDResponse<bool>> BActualizarEstatusCuenta(EntActualizarEstatusCuenta entActualizarEstatusCuenta)
        {
            IMDResponse<bool> response = new IMDResponse<bool>();
            ...
            try
            {
                if (entActualizarEstatusCuenta.iAccionCuenta == (int)eAccionCuenta.DESBLOQUEO && entActualizarEstatusCuenta.iEstatusCuenta == (int)eEstatusCuenta.REPORTADO)
                {
                    var usuarioResponse = await _datUsuario.DGet(entActualizarEstatusCuenta.uIdUsuario);

                    if (!usuarioResponse.HasError)
                    {
                        if (usuarioResponse.Result != null)
                        {
                            Guid uIdTarifaGeneral = Guid.Parse(_busParametros.BObtener("APP_TARIFAGENERAL_GUID").Result.Result.sValor);

                            var monederoResponse = _busMonedero.BDatosMonedero(usuarioResponse.Result.uIdMonedero.Value).Result;

                            if (monederoResponse.HasError)
                            {
                                response.ErrorCode = usuarioResponse.ErrorCode;
                                response.SetError(usuarioResponse.Message);
                                return response;
                            ...
                            if (monederoResponse.Result != null)
                            {
                                if (monederoResponse.Result.idTipoTarifa != uIdTarifaGeneral)
                                    entActualizarEstatusCuenta.iEstatusCuenta = (int)eEstatusCuenta.ENCAMBIO;
                                else
                                    entActualizarEstatusCuenta.iEstatusCuenta = (int)eEstatusCuenta.DESBLOQUEADO;
                            }
                            else
                            {
                                response.ErrorCode = metodo.iCodigoError;
                ...
                if (entActualizarEstatusCuenta.iAccionCuenta == (int)eAccionCuenta.BLOQUEO)
                {
                    EntDispositivoCuentaUpdate entDispositivoCuentaUpdate = new EntDispositivoCuentaUpdate()
                    {
                        uIdUsuario = entActualizarEstatusCuenta.uIdUsuario,
                        sIdAplicacion = "",
                        iEstatusCuenta = (int)eEstatusCuenta.REPORTADO
                    };

                    entActualizarEstatusCuenta.iEstatusCuenta = (int)eEstatusCuenta.REPORTADO;

                    // Cierre de sesión para todos los dispositivos.
                    BCerrarSesionDispositivos(entDispositivoCuentaUpdate);
                }

                response = await _datUsuario.DActualizarEstatusCuenta(entActualizarEstatusCuenta);
            }
            catch (Exception ex)
            {
                response.ErrorCode = 500;
        ...
        [IMDMetodo(67823465658256, 67823465657479)]
        public async Task<IMDResponse<bool>> BActualizarDatosMonedero(Guid uIdUsuario)
        {
            IMDResponse<bool> response = new IMDResponse<bool>();
            IMDMetodo metodo = MethodBase.GetCurrentMethod()!.GetIMDMetodo();
            _logger.LogInformation(IMDSerializer.Serialize(67823465657479, $"Inicia {metodo}(Guid uIdUsuario)", uIdUsuario));
            ...
            try
            {
                ...
                if (usuario.Result.uIdMonedero != null)
                {
                    var replicaMonedero = new EntReplicaUpdateMonederos
                    {
                        IdMonedero = (Guid)usuario.Result.uIdMonedero,
                        ApellidoMaterno = usuario.Result.sApellidoMaterno,
                        ApellidoPaterno = usuario.Result.sApellidoPaterno,
                        Correo = usuario.Result.sCorreo,
                        FechaNacimiento = usuario.Result.dtFechaNacimiento,
                        Nombre = usuario.Result.sNombre,
                        NumTelefono = usuario.Result.sTelefono
                    };

                    await _rabbitNotifications.SendAsync(RoutingKeys.MonederoActualizacion.GetDescription(), _exchangeConfig, new QueueMessage<EntReplicaUpdateMonederos>
                    {
                        Content = replicaMonedero
                    });
                }
                else
                {
                    response.SetError("El usuario no cuenta con monedero");
                }
            }
            catch (Exception ex)
            {
                ...
                _logger.LogError(IMDSerializer.Serialize(67823465658256, $"Error en {metodo}(Guid uIdUsuario): {ex.Message}", uIdUsuario, ex, response));
            }
            return response;
        }
        ...
        [IMDMetodo(67823466302389, 67823466303166)]
        public async Task<IMDResponse<List<EntFirebaseToken>>> BGetListFirebaseToken(Guid uIdUsuario, string sIdAplicacion)
        {
            IMDResponse<List<EntFirebaseToken>> response = new IMDResponse<List<EntFirebaseToken>>();
            ...
            try
            {
                response = await _datUsuario.DGetListFirebaseToken(uIdUsuario, sIdAplicacion);
            }
            catch (Exception ex)
            {
                response.ErrorCode = metodo.iCodigoError;
